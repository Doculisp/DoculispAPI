...
- **Token Type Understanding**: All 4 token types (text, atom, parameter, close parenthesis) and their structures
...
- Understand the `isToken` state flag that controls atom vs parameter parsing
- Handle escape sequences properly in parameters (`\(`, `\)`, `\\`)
- Leverage precise location tracking for error reporting and debugging
- Follow the parsing handler sequence (whitespace → comments → parentheses → parameters → atoms)
...
- **AST Node Types**: All AST node structures (`IAstValue`, `IAstAtom`, `IAstCommand`, `IAstContainer`, etc.)
- **Parsing Patterns**: Four distinct parsing patterns (text, atom, command, container) and their requirements
...
- **Type Hierarchies**: Understanding `AtomAst`, `CoreAst`, and `RootAst` relationships
...
- Follow the type hierarchy: `CoreAst` contains `AtomAst` and `IAstValue`
