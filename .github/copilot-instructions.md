...
**CRITICAL RULE: Never update the package version number in `package.json`**

- Package version changes are reserved for manual control by project maintainers
- The package version should always be controlled by the npm executable and always run by the user or at the direct request of the user
- Version numbers follow semantic versioning and require careful coordination with releases
- AI assistants should never modify the `version` field in `package.json` under any circumstances
- If version-related changes are needed, explicitly ask the maintainer to handle version updates using npm commands
- Focus on code changes, dependencies, and other package.json fields, but leave versioning to humans
